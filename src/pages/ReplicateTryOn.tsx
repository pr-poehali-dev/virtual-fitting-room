...
export default function ReplicateTryOn() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const balanceCheck = await checkReplicateBalance(user, selectedClothingItems?.length || 0);
      if (!balanceCheck.canGenerate) {
        return;
      }

      const balanceDeducted = await deductReplicateBalance(user, selectedClothingItems?.length || 0);
      if (!balanceDeducted) {
        return;
      }
