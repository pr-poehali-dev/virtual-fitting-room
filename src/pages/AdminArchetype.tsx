...
import { ARCHETYPE_QUESTIONS, OptionLetter } from '@/data/archetypeTest';
...
export default function AdminArchetype() {
  const [users, setUsers] = useState<User[]>([]);
  const [items, setItems] = useState<ArchetypeItem[]>([]);
  const [userFilter, setUserFilter] = useState<string>('all');
  const [isLoading, setIsLoading] = useState(true);
  const [currentPage, setCurrentPage] = useState(1);
  const [selectedItem, setSelectedItem] = useState<ArchetypeItem | null>(null);
  ...
  const fetchArchetypeHistory = async () => {
    const params = new URLSearchParams({ action: 'archetype_history' });
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error('Ошибка загрузки тестов на архетип');
      }
    } catch (error) {
      toast.error('Ошибка загрузки тестов на архетип');
    } finally {
  ...
  const renderAnswers = (item: ArchetypeItem) => {
    ...
    return (
      <div className="space-y-4">
        {ARCHETYPE_QUESTIONS.map((q, idx) => {
          const letter = item.answers?.[q.id] as OptionLetter | undefined;
    ...
    <Layout>
      <div className="container mx-auto px-4 py-8">
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1">
            <div className="mb-8">
              <h1 className="text-3xl font-bold mb-2">Тесты на архетип</h1>
            ...
            {items.length === 0 ? (
              <Card>
                <CardContent className="p-12 text-center">
                  <Icon name="Brain" className="mx-auto mb-4 text-muted-foreground" size={48} />
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b bg-muted/40 text-left">
                          ...
                          <th className="p-3 font-medium">Архетип</th>
                        ...
                        {paginatedItems.map((item) => (
                          <tr key={item.id} className="border-b hover:bg-muted/20">
                            ...
                            <td className="p-3">
                              ...
                              {item.top_names && (
                                <div className="text-xs text-muted-foreground">{item.top_names}</div>
                              )}
                            </td>
      ...
      <Dialog open={!!selectedItem} onOpenChange={(open) => !open && setSelectedItem(null)}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {selectedItem?.user_name} — {selectedItem?.top_archetype_name}
            </DialogTitle>
          </DialogHeader>
          {selectedItem && (
            <div className="space-y-4">
              <div className="text-sm text-muted-foreground">
                Аккаунт: {selectedItem.account_name} ({selectedItem.user_email}) · топ:{' '}
                {selectedItem.top_names}
              </div>
