...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    '''
    Business: Start async SeeDream 4 generation and return task_id immediately
    ...
    if len(garments) > 2:
        return {
            ...
            'headers': {'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*'},
            'isBase64Encoded': False,
            'body': json.dumps({'error': 'Максимум 2 вещи за раз для SeeDream 4'})
        }
    ...
    try:
        ...
        cursor.execute('''
            INSERT INTO seedream_tasks (id, user_id, status, person_image, garments, prompt_hints, created_at)
