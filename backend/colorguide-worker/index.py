...
def process_image_service(task_id: str, service_type: str, person_image: str, user_id, height, form_params=None, partner_image=None):
    ...
    try:
        ...
        if height:
            gemini_prompt = (
                f'РОСТ КЛИЕНТА: РОВНО {height} см. Это точная цифра от самого клиента — НЕ оценивай '
                f'рост по фотографии и в тексте ответа пиши строго "{height} см", без округления. '
