...
# Как часто складываем в базу то, что успела написать модель. Сохраняем
# часто: если заход всё же оборвётся, написанное не пропадёт
PARTIAL_SAVE_SEC = 5

# Через сколько секунд заход выходит сам, аккуратно сохранив накопленное.
# Это страховка на случай очень долгого ответа, а НЕ рабочий режим: раньше
# тут стояло 25 с по ошибочному предположению, что функцию гасят на 31-й, и
# мы сами обрывали модель перед самым ответом. Поддержка подтвердила, что
# функция спокойно работает 73 с, поэтому даём модели договорить
...
def process_image_service(task_id: str, service_type: str, person_image: str, user_id, height, form_params=None, partner_image=None):
    ...
    try:
        ...
        if height:
            gemini_prompt = (
                f'РОСТ КЛИЕНТА: РОВНО {height} см. Это точная цифра, которую сообщил сам клиент. '
                f'Единственно верное значение роста — {height} см. Запрещено измерять, прикидывать или '
                f'уточнять рост по фотографии и подставлять любое другое число. Везде, где в ответе '
                f'упоминается рост, должно стоять строго "{height} см" — без округления и без "примерно". '
                'Учитывай этот рост при подборе длины вещей, пропорций, высоты посадки, обуви и '
                'визуального баланса силуэта.\n\n'
            ) + gemini_prompt
