...
def process_image_service(task_id: str, service_type: str, person_image: str, user_id, height, form_params=None, partner_image=None):
    ...
    try:
        ...
        if height:
            gemini_prompt += (
                f'\n\nРОСТ КЛИЕНТА: {height} см. Обязательно учитывай рост при подборе: '
                ...
                f'этому росту. КРИТИЧЕСКИ ВАЖНО: рост клиента — РОВНО {height} см. Если упоминаешь рост '
                f'в тексте ответа, пиши строго "{height} см" — не округляй, не меняй цифру и НЕ оценивай '
                'рост по фотографии. Любое другое число будет ошибкой.'
            )
