...
# Анкету клиента ставим ПЕРЕД общими правилами стиля: в хвосте промпта она
# читалась как справка и проигрывала строгим правилам, из-за чего образы
# получались усреднёнными независимо от заполнения анкеты.
PARAMS_FIRST = True

# Подбор образа — творческая задача. При 0.6 модель сходилась к одному и тому
# же безопасному решению даже на разных анкетах.
TEMPERATURE = 0.85
...
def build_params_block(form_params: dict) -> str:
    ...
    if not lines:
        return ''
    return (
        'ЭТО ГЛАВНОЕ В ЗАДАЧЕ — ПЕРСОНАЛЬНЫЕ ДАННЫЕ КЛИЕНТА.\n'
        'Клиент заполнил анкету о себе. Эти данные — основа образа и имеют '
        'ПРИОРИТЕТ ВЫШЕ общих правил стиля, которые идут ниже. Общие правила '
        'нужны, чтобы образ был грамотным, но они НЕ должны стирать личность '
        'клиента и превращать образ в безликий усреднённый вариант.\n'
        'Не указанные параметры — не выдумывай.\n\n'
        + '\n'.join(lines) + '\n\n' + PARAMS_READING_RULES
    )


# Как читать анкету: без этих пояснений модель перечисляла параметры формально,
# но в самом образе они не проявлялись — результат выходил одинаковым.
PARAMS_READING_RULES = '''КАК ЧИТАТЬ ЭТУ АНКЕТУ (обязательно):
- АРХЕТИП(Ы) — это ХАРАКТЕР и НАСТРОЕНИЕ образа, а не просто слово. Из архетипа выводи: энергетику образа, степень смелости и дерзости, силуэт, выбор деталей и акцентов, уровень декоративности. Образ для Бунтаря и для Заботливого при одном и том же поводе обязан выглядеть ЗАМЕТНО ПО-РАЗНОМУ. Если архетипов несколько — возьми их сочетание как единое настроение.
- ТИПАЖ ПО КИББИ — это ЛИНИИ и КРОЙ: силуэты, длины, степень структурности или мягкости вещей, посадка, масштаб деталей и аксессуаров. Подбирай кроя строго в логике этого типажа.
- ЛЮБИМЫЕ ТКАНИ, ЦВЕТА, ОРНАМЕНТЫ — это фильтр материалов и палитры. Постарайся реально использовать любимое в образе, а не просто не противоречить ему. Нелюбимое — исключай.
- ТЕГИ-АКЦЕНТЫ И КОММЕНТАРИЙ — прямые пожелания клиента, учитывай их в первую очередь.

УЛОВИ НАСТРОЕНИЕ, А НЕ СОБИРАЙ СПИСОК (очень важно):
Клиент мог отметить МНОГО всего — все пожелания в один образ физически не поместятся, и пытаться впихнуть их все НЕ НУЖНО: получится перегруженный несвязный набор вещей. Действуй как живой стилист: пойми по всей анкете ОБЩЕЕ НАСТРОЕНИЕ и вкус человека, выбери ОДНУ доминанту (главный архетип или главный акцент) и 1-2 поддерживающих штриха, а остальное осознанно оставь за кадром. Образ должен быть цельным и явно ПЕРСОНАЛЬНЫМ — таким, чтобы клиент узнал в нём себя, а не получил универсальный вариант, который подошёл бы любому.'''
...
- СЕЗОН И ПОГОДА — ОБЯЗАТЕЛЬНО: ткань, плотность, длина рукава и слоёв должны соответствовать указанному сезону и погоде. Для ЛЕТА и жары — только лёгкие дышащие ткани (лён, хлопок, вискоза, тонкий шёлк, тонкий трикотаж), без шерсти, кашемира, плотного трикотажа, плотной верхней одежды и лишней многослойности. Для ХОЛОДА — наоборот, тёплые плотные ткани и уместный верх. Если сезон не указан — собирай образ для умеренной погоды. ВАЖНО ПРО ЛЮБИМЫЕ ТКАНИ: если любимая ткань клиента не подходит по сезону, сначала поищи её СЕЗОННЫЙ АНАЛОГ или уместную форму подачи (например, вместо плотного бархата — лёгкая ткань с похожей фактурой или бархатная деталь/аксессуар; вместо тёплой кожи летом — тонкая кожаная деталь), и только если это невозможно — замени ткань, а причину замены коротко объясни в поле "personal_fit". Молча выбрасывать пожелания клиента нельзя.
...
- ПРОВЕРКА НА ЛОГИКУ ПЕРЕД ОТВЕТОМ (ОБЯЗАТЕЛЬНО): прежде чем выдать JSON, мысленно перепроверь весь собранный образ на здравый смысл и сочетаемость. Убедись, что КАЖДЫЙ элемент (одежда, обувь, сумка, ремень, аксессуары, украшения) логично подходит по ЦВЕТУ, СТИЛЮ и МАТЕРИАЛУ к остальному образу, к поводу, к сезону и к полу клиента. Если хоть один элемент выглядит нелогично или нелепо (например, неуместный по цвету или стилю предмет, светлая сумка к строгому тёмному костюму, летний аксессуар в зимнем образе и т.п.) — ЗАМЕНИ его на уместный, и только после этого формируй итоговый JSON. ВНИМАНИЕ: эта проверка нужна против ОШИБОК, а не против индивидуальности. Не выхолащивай образ до безликого среднего варианта: смелые, характерные и небанальные решения, если они грамотны и отвечают анкете клиента, — ОСТАВЛЯЙ. Заодно перепроверь: чувствуется ли в образе заявленный архетип и учтены ли предпочтения клиента; если нет — усиль их, а не убирай.
...

{
  ...
  "personal_fit": "2-4 предложения — как именно в этом образе отражены данные из анкеты клиента: заявленный архетип (в чём он проявился), типаж по Кибби (какие линии и кроя из него взяты), любимые ткани, цвета и орнаменты. Если какое-то пожелание учесть не удалось (например, ткань не подошла по сезону) — честно скажи об этом и объясни, чем заменил. Пиши живым языком, обращаясь к клиенту",
...
def build_context_line(form_params: dict) -> str:
    ...
    # Настроение образа (архетипы и теги) — чтобы персональность доходила
    # и до картинки, а не только до текстового описания
    mood = []
    for key in ('archetypes', 'tags'):
        val = form_params.get(key)
        if isinstance(val, (list, tuple)):
            mood += [str(v).strip() for v in val if str(v or '').strip()]
    mood_line = ''
    if mood:
        mood_line = (
            'MOOD of the look (styling character, pose and overall vibe should '
            'match it): ' + ', '.join(mood[:6]) + '.\n'
        )
...
def build_image_prompt(data: dict, height: int = None, gender=None, form_params=None) -> str:
    ...
    if is_male:
        ...
        # Раньше здесь просили отдельный крупный план лица человека — рисующая
        # модель отказывалась обрабатывать такой запрос и не выдавала картинку
        # вовсе. Показываем сбоку саму косметику в подобранных оттенках:
        # макияж остаётся на лице в центре и в текстовом разборе
        side_items.append(
            'a flat-lay of makeup products in the exact shades of this makeup '
            '(for example lipstick or gloss, eyeshadow, blush), shown as closed '
            f'and open cosmetic items only, without any face: {makeup_desc}'
        )
