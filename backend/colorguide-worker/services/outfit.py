...
# же безопасному решению на разных анкетах. Но 0.85 удлиняла рассуждения
# думающей модели, и ответ переставал укладываться во время жизни функции
TEMPERATURE = 0.75
...
def build_params_block(form_params: dict) -> str:
    ...
    if not lines:
        return ''
    return (
        'ГЛАВНОЕ — АНКЕТА КЛИЕНТА. Эти данные важнее общих правил ниже: правила '
        'нужны для грамотности образа, но не должны стирать личность клиента. '
        'Не указанное — не выдумывай.\n\n'
        + '\n'.join(lines) + '\n\n' + PARAMS_READING_RULES
    )


# Как читать анкету: без пояснений модель перечисляла параметры формально,
# но в образе они не проявлялись. Формулировки держим краткими — длинный
# промпт заставляет думающую модель рассуждать дольше времени жизни функции.
PARAMS_READING_RULES = '''КАК ЧИТАТЬ АНКЕТУ:
- АРХЕТИП — характер и настроение образа: энергетика, смелость, силуэт, акценты. Образы Бунтаря и Заботливого на один повод обязаны выглядеть ЗАМЕТНО ПО-РАЗНОМУ. Несколько архетипов — бери их сочетание.
- КИББИ — линии и крой: силуэты, длины, структурность или мягкость, посадка, масштаб деталей.
- ЛЮБИМЫЕ ТКАНИ, ЦВЕТА, ОРНАМЕНТЫ — реально используй их в образе, а не просто не противоречь. Нелюбимое исключай.
- ТЕГИ И КОММЕНТАРИЙ — прямые пожелания, учитывай в первую очередь.

УЛОВИ НАСТРОЕНИЕ, А НЕ СОБИРАЙ СПИСОК: клиент мог отметить много всего, но всё в один образ не поместится — и не надо. Пойми общее настроение и вкус человека, возьми ОДНУ доминанту и 1-2 поддерживающих штриха, остальное осознанно оставь. Образ должен быть таким, чтобы клиент узнал в нём себя.'''
...
- ПРОВЕРКА НА ЛОГИКУ ПЕРЕД ОТВЕТОМ (ОБЯЗАТЕЛЬНО): прежде чем выдать JSON, мысленно перепроверь весь собранный образ на здравый смысл и сочетаемость. Убедись, что КАЖДЫЙ элемент (одежда, обувь, сумка, ремень, аксессуары, украшения) логично подходит по ЦВЕТУ, СТИЛЮ и МАТЕРИАЛУ к остальному образу, к поводу, к сезону и к полу клиента. Если хоть один элемент выглядит нелогично или нелепо (например, неуместный по цвету или стилю предмет, светлая сумка к строгому тёмному костюму, летний аксессуар в зимнем образе и т.п.) — ЗАМЕНИ его на уместный, и только после этого формируй итоговый JSON. ВАЖНО: проверка нужна против ОШИБОК, а не против индивидуальности — смелые и характерные решения, если они грамотны и отвечают анкете, ОСТАВЛЯЙ. Заодно проверь, чувствуется ли заявленный архетип; если нет — усиль его, а не убирай.
...

{
  ...
  "personal_fit": "2-3 предложения клиенту: как в образе отражены его архетип, типаж по Кибби и любимые ткани/цвета. Если пожелание учесть не удалось — скажи честно и объясни замену",
