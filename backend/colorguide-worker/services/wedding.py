...
# Анкету ставим ПЕРЕД общими правилами: в хвосте промпта она читалась как
# справка и проигрывала правилам — образы выходили типовыми независимо
# от архетипов, типажа и пожеланий
PARAMS_FIRST = True

# Свадебный образ — творческая задача: при 0.6 модель сходилась к одному и
# тому же безопасному решению на разных анкетах
TEMPERATURE = 0.85
...
def build_params_block(form_params: dict) -> str:
    ...
    return (
        ...
        'Клиент заполнил анкету о себе и своей свадьбе. Эти данные — основа '
        'образа и имеют ПРИОРИТЕТ ВЫШЕ общих правил ниже. Правила нужны, чтобы '
        'образ был грамотным, но они НЕ должны стирать личность клиента и '
        'превращать образ в типовой свадебный вариант «как у всех».\n'
        'Не указанные параметры — не выдумывай.\n\n'
        + '\n'.join(lines) + '\n\n' + PARAMS_READING_RULES + tail
    )


# Без этих пояснений модель перечисляла параметры формально, но в самом
# образе они не проявлялись — свадебные образы выходили одинаковыми.
PARAMS_READING_RULES = '''КАК ЧИТАТЬ ЭТУ АНКЕТУ (обязательно):
- АРХЕТИП(Ы) — характер и настроение образа: степень романтичности, смелости, торжественности, выбор деталей и декора. Образы для Бунтарки и для Хранительницы очага на одной и той же свадьбе обязаны выглядеть ЗАМЕТНО ПО-РАЗНОМУ.
- ТИПАЖ ПО КИББИ — линии и крой: силуэт платья или костюма, длины, степень структурности или мягкости ткани, посадка, масштаб деталей. Это главный ключ к фасону.
- ЧТО ПОДЧЕРКНУТЬ И ЧТО СКРЫТЬ — прямая просьба о фигуре, выполняй её конкретным кроем, а не общими словами.
- ЛЮБИМЫЕ ТКАНИ И ЦВЕТА, ПАЛИТРА СВАДЬБЫ — реально используй их в образе. Нежелательное исключай.
- МЕСТО, СЕЗОН, МЕСЯЦ, ВРЕМЯ СУТОК — задают ткань, плотность, длину рукава и обувь: выездная церемония на траве летом и вечернее торжество в зале зимой требуют разного.
- ОБРАЗ ПАРТНЁРА — образы пары должны сочетаться по палитре и степени торжественности.
- ОГРАНИЧЕНИЯ И БЮДЖЕТ — жёсткие рамки, нарушать нельзя.
- ТЕГИ И КОММЕНТАРИЙ — прямые пожелания клиента, учитывай в первую очередь.

УЛОВИ НАСТРОЕНИЕ, А НЕ СОБИРАЙ СПИСОК (очень важно):
Клиент мог отметить МНОГО всего — вместить все пожелания в один образ невозможно, и не нужно: получится перегруженный несвязный наряд. Действуй как живой свадебный стилист: пойми по всей анкете ОБЩЕЕ НАСТРОЕНИЕ и вкус человека, возьми ОДНУ доминанту (главный архетип или главное пожелание) и 1-2 поддерживающих штриха, остальное осознанно оставь за кадром. Свадебный образ должен быть таким, чтобы клиент узнал в нём себя в лучший день своей жизни, а не получил типовой вариант из свадебного салона.'''
...

{
  ...
  "personal_fit": "2-4 предложения клиенту: как именно в образе отражены данные его анкеты — заявленный архетип (в чём он проявился), типаж по Кибби (какие линии и крой из него взяты), любимые ткани и цвета, просьбы что подчеркнуть или скрыть. Если какое-то пожелание учесть не удалось — скажи честно и объясни, чем заменил. Пиши живым языком, обращаясь к клиенту",
...
def _clean_partner(obj):
    """Чистит от отсылок к партнёру только текст описания, не трогая структуру."""
    if isinstance(obj, dict):
        cleaned = dict(obj)
        if cleaned.get('description'):
            cleaned['description'] = _strip_partner(str(cleaned['description']))
        return cleaned
    return obj


def _clean_partner_list(items):
    return [_clean_partner(it) for it in (items or [])]
...
def build_image_prompt(data: dict, height: int = None, gender=None, form_params=None) -> str:
    ...
    hair_desc = _obj_desc(_clean_partner(data.get('hairstyle')))
    hair_line = (
    ...
    shoes_desc = _obj_desc(_clean_partner(data.get('shoes')))
    headpiece_desc = _obj_desc(_clean_partner(data.get('headpiece')))
    accessories_desc = _join_descs(_clean_partner_list(data.get('accessories')))
    jewelry_desc = _join_descs(_clean_partner_list(data.get('jewelry')))
    makeup_desc = _obj_desc(_clean_partner(data.get('makeup')))

    outfit_parts = []
    wear_desc = _join_descs(_clean_partner_list(data.get('outfit')))
    if wear_desc:
    ...
    headpiece_name = _short_name(data.get('headpiece'))
    ...
    if headpiece_name:
        ...
        side_items.append(f'{label} ({headpiece_name})')
    ...
    if is_male:
        ...
        side_items.append(
            'a flat-lay of bridal makeup products in the exact shades of this makeup '
