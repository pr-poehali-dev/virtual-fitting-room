...
def _build_rows_block(spread: dict, deck: dict, layout: list) -> str:
    ...
    def card_line(i: int, col: int) -> str:
        ...
        return (
            f'столбец {col}, в ДОМ №{i + 1} «{house}»{_mixup(house)} '
...
def build_divination_prompt(meta: dict) -> str:
    ...
    if spread.get('cards_format') == 'rows' and rows_block:
        parts.append(
            'КАРТЫ В РАСКЛАДЕ (держи этот список под рукой). Это '
            'единственный источник правды о раскладе: каждый раз, когда '
            'называешь дом и карту в этом доме, сверяйся с этим списком, '
            'а не с памятью.'
        )
